...
object Error {
  ...
  /** Used to indicate a validations error.
    *
    * @param failurePoint
    *   The extraction from where the error failed.
    * @param input
    *   The input, if available.
    * @tparam T
    *   Type that was validated.
    */
  ...
  /** Used when we receive a type that doesn't match the expected type. For instance if we are
    * expecting an Int, but we receive a String.
    *
    * @param path
    *   The path within the schema to the offending definition
    * @param expectedType
    *   The expected type.
    * @param providedType
  ...
  /** Used when we can not convert an input type to the type defined in the schema. For instance, in
    * JSON we use a String to represent an Enumeration. However, if the string does not map to a
    * known enumeration, this error would be specified.
    *
    * @param path
    *   The path within the schema to the offending definition
    * @param input
    *   The input type being converted
    * @param toType
    *   The type we are attempting to convert to.
    * @tparam A
    *   Input type
    * @tparam T
    *   Type to convert to
  ...
  /** Used when a required piece of data is missing
    ...
    *   The path within the schema to the offending definition
    * @param typeName
    *   The description of the required value
    */
  ...
  /** Used when we can not parse the input to its expected format -- for instance, an invalid JSON
    * document.
    *
    * @param message
  ...
  /** Used when an exception is thrown in processing the request in the business logic. This is a
    * reasonable exception to use as starting point for Exceptions reported in the integration
    * layer.
    *
    * @param th
    *   The Exception.
    * @param message
    *   Context specific message about the exception.
    */
  ...
  /** Used when the ID is not present in the integration layer.
    ...
    * @tparam ID
    *   The type of the ID (such as Long or UUID)
    */
