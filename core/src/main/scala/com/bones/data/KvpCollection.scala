...
/** Base Trait for anything that represents a collection KeyValue Pair. The direct subclasses are:
  * [[KvpHListCollection]] - Base Class when the A is an HList. With this we have the ability to
  * construct a new KvpCollection with a new item at the head (aka cons, aka ::) and we have the
  * ability to construct a new KvpCollection prepending another HList (aka :::) [[KvpCoproduct]] -
  * Base Class when the A is a Coproduct, aka, OneOf. This can eventually be wrapped by a value with
  * represents a single trait. [[WrappedEncoding]] - This allows us to wrap an HList type (such as
  * String :: Ing :: HNil) or a Coprodcut type (such as Cat :+: Dog :+: CNil) into more scala
  * friendly types (such as Person(name: String, age: Int) or (trait Animal) respectively.
  */
...
/** This allows us to wrap an HList type (such as String :: Ing :: HNil) or a Coprodcut type (such
  * as Cat :+: Dog :+: CNil) into more scala friendly types (such as Person(name: String, age: Int)
  * or (trait Animal) respectively.
  * @tparam ALG
  *   the GADT Context
  * @tparam A
  *   The new value which is a switch in context from the Generic type values.
  */
...
sealed trait KvpCoproduct[K, ALG[_], C <: Coproduct] extends KvpCollection[K, ALG, C] {
  ...
  def :+:[A: Manifest](
    head: KvpCollection[K, ALG, A]
  ): KvpCoproductCollectionHead[K, ALG, A, C, A :+: C] = {
  ...
  def toSuperclassOf[A: Manifest](implicit
    gen: Generic.Aux[A, C]
  ): KvpWrappedCoproduct[K, ALG, A, C] =
    toSuperclassOf[A]()
...
/** @param kvpTail
  *   The rest of the coproduct
  * @tparam ALG
  *   The GADT context.
  * @tparam A
  *   The head (or most left part) of the coproduct (of the values of Coproduct, the one in
  *   particular this instance represents)
  * @tparam C
  *   The remaining part of the coproduct. This class
  * @tparam O
  *   The new Corpoduct after the A is prepended to C.
  */
case class KvpCoproductCollectionHead[K, ALG[_], A, C <: Coproduct, O <: A :+: C](
  ...
  def algMapKvpCoproductCollectionHead[ALG2[_]](
    f: ALG[_] => ALG2[_]
  ): KvpCoproductCollectionHead[K, ALG2, A, C, Nothing] = {
    this.copy(
...
/** Contains a collection of key-value pairs.
  *
  * @tparam L
  *   The HList this value represents.
  * @tparam LL
...
sealed abstract class KvpHListCollection[K, ALG[_], L <: HList, LL <: Nat]
  ...
  def convertWithName[A](typeName: String, validation: ValidationOp[A]*)(implicit
    gen: Generic.Aux[A, L]
  ): KvpWrappedHList[K, ALG, A, L, LL] =
    KvpWrappedHList(this, typeName, gen.from, gen.to, validation.toList)

  def tupled[Tup <: Product](implicit
    tupler: Tupler.Aux[L, Tup],
    gen: Generic[Tup]
  ): KvpWrappedHList[K, ALG, Tup, L, LL] = tupled[Tup]()

  def tupled[Tup <: Product](tupledValidations: ValidationOp[Tup]*)(implicit
    tupler: Tupler.Aux[L, Tup],
    gen: Generic[Tup]
  ...
  def xmap[A: Manifest, B](f: B => A, g: A => B, validation: ValidationOp[A]*)(implicit
    isEqual1: (B :: HNil) =:= L,
    isEqual2: L =:= (B :: HNil)
  ): KvpWrappedHList[K, ALG, A, L, LL] = {
    type T = B :: HNil
  ...
  def encodedHead[B: Manifest](validation: ValidationOp[B]*)(implicit
    isEqual1: (B :: HNil) =:= L,
    isEqual2: L =:= (B :: HNil)
  ...
  def prependHList[NL <: HList, NLL <: Nat, L2 <: HList, LL2 <: Nat](
    ...
    prepend: hlist.Prepend.Aux[L2, L, NL],
    lengthP: Length.Aux[L2, LL2],
    length: Length.Aux[NL, NLL],
    split: Split.Aux[NL, LL2, L2, L]
  ): KvpHListCollection[K, ALG, NL, NLL] = {
    val typeName = "H"
  ...
  def consKeyDefinition[A: Manifest](
    v: KeyDefinition[K, ALG, A]
  ...
  def consKvpCollection[X: Manifest](
    kvpCollection: KvpCollection[K, ALG, X]
  ...
  def >>:[A: Manifest](v: KeyDefinition[K, ALG, A])(implicit
    isHCons: IsHCons.Aux[A :: L, A, L]
  ): KvpSingleValueHead[K, ALG, A, L, LL, A :: L] =
    consKeyDefinition(v)

  /** Alias for prependCoproduct */
  def ::[X: Manifest](coproduct: KvpCollection[K, ALG, X])(implicit
    isHCons: IsHCons.Aux[X :: L, X, L]
  ): KvpSingleValueHead[K, ALG, X, L, LL, X :: L] =
    consKvpCollection(coproduct)

  /** Use this operator when you want to prefix a Data Type.
    * @param input
    *   The Key, Value Pair to Add to the front of the KvpHList
    * @param isHCons
    *   The implied ability to cons (and unapply) A to and from H
    * @tparam A
    *   The wrapped type
    * @return
    *   KvpSingleValueHead prefixed to this HList
    */
  def ::[A: Manifest](
    input: (K, ALG[A])
  )(implicit isHCons: IsHCons.Aux[A :: L, A, L]): KvpSingleValueHead[K, ALG, A, L, LL, A :: L] = {
  ...
  /** Use this operator when you want to prefix a data type with a description
    * @param input
    *   The Key, Value Pair and Description, Example to Add to the front of the KvpHList
    * @param isHCons
    *   The implied ability to cons (and unapply) A to and from H
    * @tparam A
    *   The wrapped type
  ...
  /** Prefixes one of the Algebra types to this HLIst
    ...
    */
  def :<:[A: Manifest](
    input: (K, HigherOrderValue[K, ALG, A])
  ...
  def :<:[A: Manifest](
    input: (K, HigherOrderValue[K, ALG, A], String, A)
  )(implicit isHCons: Aux[A :: L, A, L]): KvpSingleValueHead[K, ALG, A, L, LL, A :: L] = {
    val typeName = manifest[A].runtimeClass.getName.split('$').lastOption.getOrElse("")
    consKeyDefinition(
...
final case class KvpSingleValueHead[K, ALG[_], X, XS <: HList, XSL <: Nat, OUT <: X :: XS](
  ...
  def algMapKvpSingleValueHead[ALG2[_]](
    f: ALG[_] => ALG2[_]
  ): KvpHListCollection[K, ALG2, OUT, Succ[XSL]] = {
    val newHead = head match {
...
final case class KvpHListCollectionHead[K, ALG[
  _
], HO <: HList, NO <: Nat, H <: HList, HL <: Nat, T <: HList, TL <: Nat](
  ...
  split: Split.Aux[
    ...
    H,
    T
  ], // analogous: Split.Aux[prepend.OUT,HL,H,T] with lpLength: Length.Aux[H,HL],
  ...
  def keyMapKvpHListCollectionHead[KK](
    f: K => KK
  ): KvpHListCollectionHead[KK, ALG, HO, NO, H, HL, T, TL] =
  ...
  def algMapKvpHListCollectionHead[ALG2[_]](
    f: ALG[_] => ALG2[_]
  ): KvpHListCollectionHead[K, ALG2, HO, NO, H, HL, T, TL] =
