...
package object values {
  ...
  type AnyAlg[_] =
    Any // when an algebra interpreter can accept any algebra (for instance when the output is based on the keys and not the values)
  ...
  /** This is to allow smart constructors for each GADT, so that the data structure is lifted into
    * the context of [[DefaultValues]] type. For example, the smart constructor `email` for creating
    * an email data type would become Inr(Inl(EmailData()) which satisfies the [[DefaultValues]]
    * definition.
    */
