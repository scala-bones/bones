...
trait ToCollectionData[B]
  extends ToOptionalData[B] with ToListData[B] { self: KvpValue[B] =>
...
trait AlgToCollectionData[ALG[_], B, SELF <: CustomAlgebraWithManifest[ALG, B]]
  extends AlgToOptionalData[ALG, B, SELF] with AlgToListData[ALG, B, SELF] { self: SELF =>
}
