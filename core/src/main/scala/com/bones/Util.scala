...
object Util {
  ...
  def stringToUuid(uuidString: String, path: List[String]): Either[NonEmptyList[ExtractionError], UUID] = try {
  ...
  def stringToZonedDateTime(input: String, dateFormat: DateTimeFormatter, path: List[String]): Either[NonEmptyList[ExtractionError], ZonedDateTime] = try {
    Right(ZonedDateTime.parse(input, dateFormat))
  } catch {
    case NonFatal(ex) => Left(NonEmptyList.one(CanNotConvert(path, input, classOf[Date])))
  }

  def stringToBigDecimal(input: String, path: List[String]): Either[NonEmptyList[ExtractionError], BigDecimal] = try {
    Right(BigDecimal(input))
  } catch {
    case _: NumberFormatException => Left(NonEmptyList.one(CanNotConvert(path, input, classOf[BigDecimal])))
  }

  def stringToEnumeration[A](str: String, path: List[String], enumeration: Enumeration, manifest: Manifest[A]): Either[NonEmptyList[CanNotConvert[String,Object]],A] = try {
  ...
  def stringToEnum[A <: Enum[A]](str: String, path: List[String], enums: List[A]): Either[NonEmptyList[CanNotConvert[String,Object]],A] =
    enums
      .find(_.toString == str)
      .toRight(NonEmptyList.one(CanNotConvert(path, str, classOf[Object])))

  def eitherMap2[A,B,Z](e1: Either[NonEmptyList[ExtractionError], A], e2: Either[NonEmptyList[ExtractionError], B])
