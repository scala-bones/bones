...
object JavaTimeValidation {
  ...
  object InstantValidation
    extends BaseDateValidation[Instant]
  ...
  object OffsetDateTimeValidations extends BaseDateValidation[OffsetDateTime] {
    override def defaultFormatToString(f: OffsetDateTime): String = DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(f)
  ...
  object OffsetTimeValidations extends BaseDateValidation[OffsetTime] {
    override def defaultFormatToString(f: OffsetTime): String = DateTimeFormatter.ISO_OFFSET_TIME.format(f)
  ...
  object PeriodValidations extends BaseValidationOp[Period] with OrderingValidation[Period] with ZeroValidations[Period] {
    /** What is the zero Value of N.  Used for positive and negative comparisons */
  ...
  object YearValidations extends BaseValidationOp[Year] with OrderingValidation[Year] with ZeroValidations[Year] {
    /** What is the zero Value of N.  Used for positive and negative comparisons */
  ...
  object YearMonthValidations extends BaseValidationOp[YearMonth] with OrderingValidation[YearMonth] {
    override def compare(x: YearMonth, y: YearMonth): Int = x.compareTo(y)
  }

  object ZoneIdValidations extends BaseValidationOp[ZoneId]

  object ZoneOffsetValidations extends BaseDateValidation[ZoneOffset] with ZeroValidations[ZoneOffset] {
  ...
  object ZonedDateTimeValidations extends BaseDateValidation[ZonedDateTime] {
    ...
    override def defaultFormatToString(f: ZonedDateTime): String = DateTimeFormatter.ISO_ZONED_DATE_TIME.format(f)
