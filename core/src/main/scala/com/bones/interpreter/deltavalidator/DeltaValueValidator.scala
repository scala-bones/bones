...
/**
  ...
  * @tparam A The result if validation is successful
  * @tparam IN The input interchange format, such as a JSON blob.
  */
trait DeltaValueValidator[K, ALG[_], A, IN] { self =>
   ...
   **/
  def addValidation(
    f: (A, List[K]) => Either[ExtractionErrors[K], A]): DeltaValueValidator[K, ALG, A, IN] =
  ...
  def flatMap[B](f: CanBeOmitted[K, A] => DeltaValueValidator[K, ALG, B, IN])
