...
import com.bones.data.Error.{RequiredValue, ValidationError}
...
trait CustomStringValidator[OUT] extends InterchangeFormatValidator[CustomStringValue, OUT] {
  ...
  override def validate[A](alg: CustomStringValue[A]): (Option[OUT], List[String]) => Either[NonEmptyList[Error.ExtractionError], A] = {
        ...
        case Some(json) =>
          baseValidator
            ...
            .flatMap(result => {
              ...
              ValidationUtil.validate[String](allValidations)(result, path).asInstanceOf[Either[NonEmptyList[Error.ExtractionError], A]]
            })
        case None =>
