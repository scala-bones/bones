...
trait KvpInterchangeFormatEncoderInterpreter[ALG[_], OUT]
  ...
  /** This is the main entry point whose purpose is to convert a schema into a function which
    * expects the Type described by the schema (for example a case class) and converts it into an
    * interchange format library's data structure (such as Circe JSON). This interpreter assumes the
    * data has already been validated and is only responsible for the data conversion.
    *
    * @param collection
    * @tparam A
    * @return
  ...
  /** Takes a value definition and the actual value and create a key value pair wrapped in the OUT
    * type. Analogous to wrapping a key value pair in a JSON Object.
    */
