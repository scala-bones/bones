...
trait ScalaCoreResultSetInterpreter extends ResultSetValueInterpreter[ScalaCoreValue] {
  override def resultSet[A](alg: ScalaCoreValue[A]): (Path, FieldName) => ResultSet => Either[NonEmptyList[Error.ExtractionError], A] =
    alg match {
      case ob: BooleanData =>
        (path, fieldName) => rs =>
