...
/** There are three ways a column can be part of a group.
  *   1. It can be solo unique, which is a unique constraint on a value w/o a group name or with a
  *      group name, but without a partner. 2.It can be grouped unique, which has a group name and a
  *      partner. 3.It can be part of a larger unique construct, such as if an entire HList or Class
  *      is designated as unique.
  *
  * For 1 and 2 above, the IdealColumn wil be added to the UniqueGroup list at the time the
  * IdealColumn is created in the Value Interpreter. For 3 above, we will search for all new columns
  * added to the child of a group and add those columns to the UniqueGroup list.
  *
  * @tparam ALG
  */
trait Ideal[ALG[_]]
  ...
  val addColumnToWorkingTable = new HigherOrderTemplate[
    ...
    override protected def optionalToOut[B](
      opt: OptionalValue[String, ALG, B]
    ): (TableCollection, List[UniqueGroup], ColumnName, Option[Description]) => (
      TableCollection,
      List[UniqueGroup]
    ) = {
    ...
    private def nullableNewColumns(
      collection: TableCollection
    ...
    /** If Either is a top-level entry point, then we will create two tables. If it is not, we will
      * combine the two resulting tables, passing the names 'left' and 'right' as prefixes to the
      * column names.
      * @param either
      *   The data describing the either.
      * @tparam A
      *   The left type.
      * @tparam B
      *   The right type
      * @return
      *   A function to create the tables.
      */
    override protected def eitherToOut[A, B](
      either: EitherData[String, ALG, A, B]
    ): (TableCollection, List[UniqueGroup], ColumnName, Option[Description]) => (
      TableCollection,
      List[UniqueGroup]
    ) = {
      ...
      (tc, ug, columnName, desc) => {
        ...
        val newColumns =
          aMatching.map(col =>
            col.copy(name = col.name + "_" + camelToSnake(either.typeNameOfA))
          ) :::
            aUnique :::
            bMatching.map(col =>
    ...
    override protected def listToOut[A](
      list: ListData[String, ALG, A]
    ): (TableCollection, List[UniqueGroup], ColumnName, Option[Description]) => (
      TableCollection,
      List[UniqueGroup]
    ) = {
    ...
    /** Create a function, where it he column name is provided, we will create a new table
      * collection.
      * @param hList
      * @return
      */
    override protected def kvpCollectionToOut[A](
      hList: KvpCollectionValue[String, ALG, A]
    ): (TableCollection, List[UniqueGroup], ColumnName, Option[Description]) => (
      TableCollection,
      List[UniqueGroup]
    ) = {
    ...
    value match {
      ...
      case kvp: KvpCoproductCollectionHead[String, ALG, a, C, o] @unchecked => {
        ...
        (tc, ug, cn, de) => {
          ...
          val newTailColumns =
            tailTc.activeTable.columns.filterNot(col => headTc.activeTable.columns.exists(_ eq col))
