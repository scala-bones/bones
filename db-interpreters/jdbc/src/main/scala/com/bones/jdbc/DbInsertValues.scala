...
object DbInsertValues {
  ...
  def insertQuery[ALG[_], A](bonesSchema: BonesSchema[ALG,A], customInterpreter: CustomInterpreter[ALG]): DataSource => A => Either[SystemError, (ID,A)] = {
  ...
  def insertQueryWithConnection[ALG[_],A](bonesSchema: BonesSchema[NoAlgebra,A]): A => Connection => Either[SystemError, (ID,A) ] =
    insertQueryWithConnectionCustomAlgebra(bonesSchema, NoAlgebraCustomInterpreter)

  def insertQueryWithConnectionCustomAlgebra[ALG[_],A](bonesSchema: BonesSchema[ALG,A], customInterpreter: CustomInterpreter[ALG]): A => Connection => Either[SystemError, (ID,A) ] =
    bonesSchema match {
  ...
  def kvpHList[ALG[_],H<:HList,HL<:Nat](group: KvpHList[ALG,H,HL], customInterpreter: CustomInterpreter[ALG]): (Index, H) => (Index, List[(ColumnName, SetValue)]) = {
    group match {
      case nil:KvpNil[_] => (i,h) => (i, List.empty)
      case op: KvpSingleValueHead[ALG, h, t, tl, H] @unchecked => {
        val headF = determineValueDefinition(op.fieldDefinition.op, customInterpreter)(op.fieldDefinition.key)
  ...
  private def fromBonesSchema[ALG[_],A](bonesSchema: BonesSchema[ALG,A], customInterpreter: CustomInterpreter[ALG]):
  ...
  def determineValueDefinition[ALG[_],A](coproduct: CoproductDataDefinition[ALG, A], customInterpreter: CustomInterpreter[ALG]): InsertPair[A] = {
    coproduct match {
  ...
  def valueDefinition[ALG[_],A](fgo: KvpValue[A], customInterpreter: CustomInterpreter[ALG]): InsertPair[A] =
    fgo match {
