...
object FieldNames {
  ...
  def fromCustomSchema[ALG[_],A](dc: BonesSchema[ALG, A], customFieldNamesInterpreter: CustomFieldNamesInterpreter[ALG]): List[String] =
    dc match {
      case t: HListConvert[ALG, a, al, b] @unchecked => kvpHList(t.from, customFieldNamesInterpreter)
    }

  def kvpHList[ALG[_], H <: HList, HL <: Nat](group: KvpHList[ALG, H, HL], customFieldNamesInterpreter: CustomFieldNamesInterpreter[ALG]): List[String] =
    group match {
      case nil: KvpNil[_] => List.empty
      case op: KvpSingleValueHead[ALG, h, t, tl, a] @unchecked=>
        List(camelToSnake(op.fieldDefinition.key)) ::: kvpHList(op.tail, customFieldNamesInterpreter)
      case op: KvpConcreteTypeHead[ALG, a, ht, nt] @unchecked => {
        val headList = op.bonesSchema match {
          case hList: HListConvert[ALG, h, n, a] => kvpHList(hList.from, customFieldNamesInterpreter)
      ...
      case op: KvpHListHead[ALG, a, al, h, hl, t, tl] @unchecked =>
        kvpHList(op.head, customFieldNamesInterpreter) ::: kvpHList(op.tail, customFieldNamesInterpreter)
    }

  def determineValueDefinition[ALG[_], A](valueDefinitionOp: Either[KvpValue[A], ALG[A]], customFieldNamesInterpreter: CustomFieldNamesInterpreter[ALG]): List[String] =
  ...
  def valueDefinition[ALG[_], A](fgo: KvpValue[A], customFieldNamesInterpreter: CustomFieldNamesInterpreter[ALG]): List[String] =
    fgo match {
      ...
      case kvp: KvpHListValue[ALG, h, hl] @unchecked => kvpHList(kvp.kvpHList, customFieldNamesInterpreter)
