...
object DbUtil {
  ...
  def withDataSource[A](ds: DataSource)(f: Connection => Either[NonEmptyList[ExtractionError], A]): Either[NonEmptyList[ExtractionError],A] =
    try {
  ...
  def withStatement[A](con: CallableStatement)(f: CallableStatement => Either[NonEmptyList[ExtractionError],A]): Either[NonEmptyList[ExtractionError],A] =
    try {
      ...
      case NonFatal(th) => Left(NonEmptyList.one(SystemError(List.empty, th, Some("Error with CallableStatement"))))
    } finally {
      con.close()
    }
