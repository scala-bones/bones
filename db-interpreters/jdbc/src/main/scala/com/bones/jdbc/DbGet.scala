...
object DbGet {
  ...
  type DbGetCustomInterpreter[ALG[_]] = ColumnNameCustomInterpreter[ALG] with ResultSetCustomInterpreter[ALG]

  object NoAlgebraDbGetCustomInterpreter extends ColumnNameCustomInterpreter[NoAlgebra] with ResultSetCustomInterpreter[NoAlgebra] {
    def keyToColumnNames[A](alg: NoAlgebra[A]): ColumnNameInterpreter.Key => List[ColumnNameInterpreter.ColumnName] = 
  ...
  : Long => Connection => Either[NonEmptyList[ExtractionError],(Long,A)] = {
    schema match {
      case xMap: HListConvert[ALG, a, al, b] => {
        ...
        val schemaWithId = (DbUtil.longIdKeyValueDef[ALG] >>: schema :><: com.bones.syntax.kvpNilCov[ALG]).tupled[(Long,A)]
