...
trait GenGadt {
  ...
  def genHListValue(): Gen[KvpCollectionValue[ALG, _]] =
    ...
    } yield KvpCollectionValue(hList, List.empty)
  ...
  def nextGen[A, H <: HList, N <: Nat](
    ...
    implicit val manifestOfA = newHead.manifestOfA

    val isHCons = implicitly[IsHCons.Aux[A :: H, A, H]]
    val thisValue: KvpHListCollection[ALG, A :: H, Succ[N]] = KvpSingleValueHead(
      Left(KeyDefinition[ALG, A](key, Right(newHead), Some("Schema generated by Gen GADT"), None)),
      List.empty,
      tail,
      isHCons)
    if (done) {
      ...
      nextType match {
        ...
        case "Object" => {
          genKvpSingleValueHead(StringData(List.empty), KvpNil[ALG]()).flatMap(kvpHList => {
            val next = KvpCollectionValue(kvpHList, List.empty)
            genKeys.map(objKey => {
              KvpSingleValueHead(
                Left(KeyDefinition(key, Left(next), Some("Schema generated by Gen GADT"), None)),
                List.empty,
                thisValue,
                null)
            })
