...
class LiftJsonTest extends FunSuite with Checkers with MustMatchers {

  implicit override val generatorDrivenConfig =
    PropertyCheckConfiguration(minSuccessful = 1000, workers = 5)

  val validateFromCirce = ValidatedFromJObjectInterpreter.isoInterpreter

  val jsonToCc = validateFromCirce.byteArrayFuncFromSchema(allSupportCaseClass, Charset.forName("UTF8"))
  val ccToJson = EncodeToJValueInterpreter.isoInterpreter.fromSchema(allSupportCaseClass)
  ...
  test("scalacheck allSupport types - marshall then marshall") {
    check((cc: AllSupported) => {
      ...
      val newCc = jsonToCc(jsonString.getBytes(Charset.forName("UTF8")))
      newCc match {
        case Left(x) =>
          fail(s"expected success, received $x for JSON string ${jsonString}")
        case Right(newCc2) =>
