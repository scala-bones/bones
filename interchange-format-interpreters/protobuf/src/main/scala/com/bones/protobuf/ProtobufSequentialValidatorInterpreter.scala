...
object ProtobufSequentialValidatorInterpreter {
  ...
  def booleanData[ALG[_], A](validations: List[ValidationOp[Boolean]]): ExtractFromProto[Boolean] =
    (fieldNumber: LastFieldNumber, path: Path[String]) => {
      ...
      (
        List(thisTag),
        fieldNumber + 1,
        (canReadTag: CanReadTag, in: CodedInputStream) =>
          {
  ...
  /** Returns a function which reads the next tag as Short data from the Coded Input Stream. The
    * returned function returns an error if the value is not a short.
    */
  ...
  /** Returns a function which reads the next tag as an Int from the Coded Input Stream. The
    * returned function returns an ExtractionError if the value is not an Int.
    */
        ...
        (canReadTag, in) =>
          {
            if (in.getLastTag == thisTag) {
              ...
              (
                true,
                intData.flatMap(i => f(i, path)).flatMap(i => vu.validate(validations)(i, path))
              )
            } else {
  ...
  def byteArrayData[ALG[_], A](
    validations: List[ValidationOp[Array[Byte]]]
  ): ExtractFromProto[Array[Byte]] =
    (fieldNumber: LastFieldNumber, path: Path[String]) => {
...
trait ProtobufSequentialValidatorInterpreter[ALG[_]] {
  ...
  /** Determine if we use the core algebra or the custom algebra and then pass control to the
  ...
  def determineValueDefinition[A](
    ...
    definition match {
      ...
      case Right(alg) =>
        (lastItem, path) => customInterpreter.extractFromProto(alg).apply(lastItem, key :: path)
    }
          ...
          (
            ...
            (canReadTag, in) => {
              ...
              val totalResult = Util
                .eitherMap2(headResult, tailResult)((l1: h, l2: t) => op.prepend(l1, l2))
                .flatMap { l =>
                  vu.validate(op.validations)(l, path)
                }
    ...
    (last: LastFieldNumber, path: Path[String]) =>
      {
        ...
        (
          List(tag),
          lastFieldNumber,
          (canReadInput: CanReadTag, in: CodedInputStream) =>
            {
              val length = in.readRawVarint32()
              val oldLimit = in.pushLimit(length)
