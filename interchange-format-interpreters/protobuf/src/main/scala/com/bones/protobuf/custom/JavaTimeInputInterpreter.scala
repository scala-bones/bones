...
import com.bones.protobuf.ProtobufSequentialInputInterpreter.{CustomInterpreter => CustomSequentialInterpreter}
...
object JavaTimeInputInterpreter extends JavaTimeValueSugar with Sugar {
  ...
  val stringToDateTimeException: (String, List[String]) => Either[NonEmptyList[ExtractionError], DateTimeException] =
  ...
  val stringToDuration: (String, List[String]) => Either[NonEmptyList[ExtractionError], Duration] =
    (str, path) =>
      Try { Duration.parse(str) }
        .toEither
        .left
        .map(th => NonEmptyList.one(CanNotConvert(path, str, classOf[Duration], Some(th))))

  val timestampToInstant: (Long,Int, List[String]) => Either[NonEmptyList[ExtractionError], Instant] =
    (seconds, nanos, path) =>
      Try { Instant.ofEpochSecond(seconds, nanos) }
        .toEither
        .left
        .map(th => NonEmptyList.one(CanNotConvert(path, (seconds,nanos), classOf[Duration], Some(th))))
  ...
  val intToMonthDay: (Int, List[String]) => Either[NonEmptyList[ExtractionError], MonthDay] =
    (int, path) =>
      ...
      } .toEither.left.map(th => NonEmptyList.one(CanNotConvert(path, int, classOf[MonthDay], Some(th))))
  ...
  val intToZoneOffset: (Int, List[String]) => Either[NonEmptyList[ExtractionError],ZoneOffset] =
    (int, path) =>
      Try { ZoneOffset.ofTotalSeconds(int)}.toEither.left.map(th => NonEmptyList.one(CanNotConvert(path, int, classOf[Month], Some(th))))

  val stringToPeriod: (String, List[String]) => Either[NonEmptyList[ExtractionError], Period] =
    (string, path) =>
      Try { Period.parse(string) }.toEither.left.map(th => NonEmptyList.one(CanNotConvert(path, string, classOf[Period], Some(th))))

  val intToYear: (Int, List[String]) => Either[NonEmptyList[ExtractionError], Year] =
    (int, path) =>
      Try { Year.of(int)}.toEither.left.map(th => NonEmptyList.one(CanNotConvert(path, int, classOf[Period], Some(th))))
  ...
  val longToYearMonth: (Long, List[String]) => Either[NonEmptyList[ExtractionError],YearMonth] =
    ...
    }.toEither.left.map(th => NonEmptyList.one(CanNotConvert(path, long, classOf[YearMonth], Some(th))))
  ...
  val stringToZoneId: (String, List[String]) => Either[NonEmptyList[ExtractionError], ZoneId] =
    (string, path) => Try { ZoneId.of(string) }.toEither.left.map(th => NonEmptyList.one(CanNotConvert(path, string, classOf[ZoneId], Some(th))))
...
trait JavaTimeOutputInterpreter extends ProtobufSequentialOutputInterpreter.CustomInterpreter[JavaTimeValue] {
...
trait JavaTimeProtoFileInterpreter extends ProtoFileInterpreter.CustomInterpreter[JavaTimeValue] {
  ...
  override def toMessageField[A](alg: JavaTimeValue[A]):
    (Name, Int) => (ProtoFileInterpreter.MessageField, Vector[ProtoFileInterpreter.NestedType], Int) =
    (name, index) => {
      alg match {
