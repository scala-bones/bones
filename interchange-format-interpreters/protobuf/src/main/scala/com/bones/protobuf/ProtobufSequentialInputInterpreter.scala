...
(
  ...
  try {
    ...
    case e: IOException => Left(NonEmptyList.one(CanNotConvert(path, in, clazz, Some(e))))
  }
  ...
  (
    ...
    val extractA: ExtractFromProto[B] = determineValueDefinition[ALG,B](ed.definitionA, valueDefinitionB, customInterpreter)
    val extractB: ExtractFromProto[C] = determineValueDefinition[ALG,C](ed.definitionB, valueDefinitionC, customInterpreter)
    (lastFieldNumber, path) => {
  ...
  (
    ...
    kvpCoproduct: (KvpCoproduct[ALG, C], KvpValue[C], CustomInterpreter[ALG]) => ExtractProductFromProto[C],
    customInterpreter: CustomInterpreter[ALG]
  ): ExtractFromProto[A] = {
  ...
  (
    ...
    kvpCoproduct: (KvpCoproduct[ALG, C], KvpValue[A], CustomInterpreter[ALG]) => ExtractProductFromProto[C],
    customInterpreter: CustomInterpreter[ALG]
  ): ExtractFromProto[A] = {
  ...
  def booleanData[ALG[_],A](coproductDataDefinition: CoproductDataDefinition[ALG, A]): ExtractFromProto[Boolean] =
    (fieldNumber: LastFieldNumber, path: Path) =>
    {
  ...
  def stringData[ALG[_]](coproductDataDefinition: CoproductDataDefinition[ALG, String]): ExtractFromProto[String] =
    stringDataWithFlatMap(coproductDataDefinition, identitySuccess)

  def stringDataWithFlatMap[ALG[_],A](coproductDataDefinition: CoproductDataDefinition[ALG, A], f: (String, Path) => Either[NonEmptyList[ExtractionError], A]): ExtractFromProto[A] =
    (fieldNumber: LastFieldNumber, path: Path) =>
    {
      ...
      (List(thisTag), fieldNumber + 1, (canReadTag, in) => {
        ...
        if (lastTag == thisTag) {
          val utfStringResult = convert(in, classOf[String], path)(cis => cis.readStringRequireUtf8())
  ...
  */
  def shortData[ALG[_]](coproductDataDefinition: CoproductDataDefinition[ALG, Short]): ExtractFromProto[Short] =
    intDataWithFlatMap(coproductDataDefinition, (a, _) => Right(a.toShort))

//  def shortDataWithFlatMap[ALG[_], A](coproductDataDefinition: CoproductDataDefinition[ALG,A], f: (Short, Path) => Either[NonEmptyList[ExtractionError], A]): ExtractFromProto[A] =
//    (fieldNumber: LastFieldNumber, path: Path) =>
//    {
//      val thisTag = fieldNumber << 3 | VARINT
//      (List(thisTag), fieldNumber + 1, (canReadTag, in) => {
//        if (in.getLastTag == thisTag) {
//          val shortResult = convert(in, classOf[Short], path)(_.readInt32().toShort)
//          (true, shortResult.flatMap(s => f(s,path)))
//        } else {
//          (canReadTag, Left(NonEmptyList.one(RequiredData(path, coproductDataDefinition))))
//        }
//      }:(CanReadTag, Either[NonEmptyList[ExtractionError], A]))
//    }
  ...
  */
  ...
  def intData[ALG[_]](coproductDataDefinition: CoproductDataDefinition[ALG, Int]): ExtractFromProto[Int] =
  ...
  def longData[ALG[_]](coproductDataDefinition: CoproductDataDefinition[ALG, Long])  : ExtractFromProto[Long] =
    longDataWithFlatMap(coproductDataDefinition, identitySuccess)

  def longDataWithFlatMap[ALG[_],A](coproductDataDefinition: CoproductDataDefinition[ALG, A], f: (Long, Path) => Either[NonEmptyList[ExtractionError],A])  : ExtractFromProto[A] =
    (fieldNumber: LastFieldNumber, path: Path) =>
    {
      val thisTag = fieldNumber << 3 | VARINT
      (List(thisTag), fieldNumber + 1, (canReadTag, in) => {
  ...
  def byteArrayData[ALG[_],A](coproductDataDefinition: CoproductDataDefinition[ALG, A])  : ExtractFromProto[Array[Byte]] =
    (fieldNumber: LastFieldNumber, path: Path) =>
    {
  ...
  def localDateTimeData[ALG[_]](coproductDataDefinition: CoproductDataDefinition[ALG, LocalDateTime], zoneOffset: ZoneOffset)  : ExtractFromProto[LocalDateTime] = {
    def f(seconds: Long, nanos: Int, path: Path): Either[NonEmptyList[ExtractionError], LocalDateTime] =
      Try { LocalDateTime.ofEpochSecond(seconds, nanos, zoneOffset) }
        .toEither.left.map(err => NonEmptyList.one(CanNotConvert(path, (seconds,nanos), classOf[LocalDateTime], Some(err))))
  ...
  (fieldNumber: LastFieldNumber, path: Path) =>
  {
    ...
    (List(thisTag), fieldNumber + 1, (canReadTag: CanReadTag, in: CodedInputStream) => {
      ...
      try {
        ...
        case ex: InvalidProtocolBufferException => {
          in.getLastTag
          (canReadTag, Left(NonEmptyList.one(WrongTypeError(path, classOf[HList], classOf[Any], Some(ex)))))
        }
        case io: IOException => {
          in.getLastTag
          (canReadTag, Left(NonEmptyList.one(WrongTypeError(path, classOf[HList], classOf[Any], Some(io)))))
        }
      }
    }:(CanReadTag, Either[NonEmptyList[ExtractionError], A]))
  }

  def floatData[ALG[_],A](coproductDataDefinition: CoproductDataDefinition[ALG, A])  : ExtractFromProto[Float] =
    (fieldNumber: LastFieldNumber, path: Path) =>
    {
  ...
  def doubleData[ALG[_],A](coproductDataDefinition: CoproductDataDefinition[ALG, A])  : ExtractFromProto[Double] =
    (fieldNumber: LastFieldNumber, path: Path) =>
    {
...
trait ProtobufSequentialInputInterpreter {
  ...
  def fromBytes[A](dc: BonesSchema[NoAlgebra, A]): Array[Byte] => Either[NonEmptyList[ExtractionError], A]=
  ...
  def valueDefinition[ALG[_], A]: (KvpValue[A], CustomInterpreter[ALG]) => ExtractFromProto[A] =
    (fgo: KvpValue[A], customInterpreter: CustomInterpreter[ALG]) =>
      fgo match {
        ...
        case bd: BigDecimalData => stringDataWithFlatMap(Left(bd), stringToBigDecimal)
        case ld: ListData[ALG, t] @unchecked => listData[ALG,t](ld, valueDefinition, customInterpreter)
        case ed: EitherData[ALG, a, b] @unchecked => eitherData[ALG,a,b](ed, valueDefinition, valueDefinition, customInterpreter)
        case esd: EnumerationData[e,a] => stringDataWithFlatMap(Left(esd), (str, path) => stringToEnumeration(str,path,esd.enumeration)(esd.manifestOfA).map(_.asInstanceOf[A]))
        case kvp: KvpCoproductValue[ALG, c] @unchecked => kvpCoproductValueData[ALG,A,c](kvp, kvpCoproduct, customInterpreter)
        case kvp: KvpHListValue[ALG, h, hl] @unchecked => kvpHListValue[ALG,A,h,hl](kvp, kvpHList, customInterpreter)
        case kvp: HListConvert[ALG, h, hl, b] @unchecked => hListConvert[ALG,b, h,hl](kvp, kvpHList, customInterpreter)
        case co: KvpCoproductConvert[ALG, c,a] @unchecked => kvpCoproductConvert[ALG,c,a](co, kvpCoproduct, customInterpreter)
      }
}
