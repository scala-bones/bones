...
trait BsonJavaTimeValidator extends InterchangeFormatValidatorValue[JavaTimeValue, BSONValue] {
  ...
  def extractLocalTime[ALG[_], A](op: ALG[A])(
    ...
    in match {
      ...
      case x              => baseValidator.invalidValue(x, classOf[BSONLong], path)
    }
  ...
  override def validate[A](alg: JavaTimeValue[A])
    ...
    alg match {
      ...
      case ld: LocalDateTimeData =>
        baseValidator.required(Right(ld), ld.validations, extractLocalDateTime(ld))
      case ld: LocalDateData =>
        baseValidator.required(Right(ld), ld.validations, extractLocalDate(ld))
      case lt: LocalTimeData =>
        baseValidator.required(Right(lt), lt.validations, extractLocalTime(lt))
      case md: MonthData =>
        parseTime(baseValidator, alg, classOf[Month], Month.valueOf, md.validations)
      case md: MonthDayData =>
        parseTime(baseValidator, alg, classOf[MonthDay], MonthDay.parse, md.validations)
      ...
      case ot: OffsetTimeData =>
        parseTime(
          ...
          ot.validations)
      case pd: PeriodData =>
        parseTime(baseValidator, alg, classOf[Period], Period.parse, pd.validations)
      case yd: YearData => parseYear(baseValidator, alg, yd.validations)
      case ym: YearMonthData =>
        parseTime(baseValidator, alg, classOf[YearMonth], YearMonth.parse, ym.validations)
      case zi: ZoneIdData =>
        parseTime(baseValidator, alg, classOf[ZoneId], ZoneId.of, zi.validations)
