  ...
  override def encode[A](alg: JavaTimeValue[A]): A => Json =
    alg match {
      case DateTimeExceptionData(_) =>
        ex =>
          Json.jString(ex.getMessage())
      case DayOfWeekData(_) =>
        dow =>
          Json.jString(dow.toString)
      case DurationData(_) =>
        duration =>
          Json.jString(duration.toString)
      case InstantData(_) =>
        instant =>
          Json.jString(instantFormatter.format(instant))
      case MonthData(_) =>
        month =>
          Json.jString(month.toString)
      case MonthDayData(_) =>
        monthDay =>
          Json.jString(monthDay.toString)
      case OffsetDateTimeData(_) =>
        offset =>
          Json.jString(offsetDateTimeFormatter.format(offset))
      case OffsetTimeData(_) =>
        offset =>
          Json.jString(offsetTimeFormatter.format(offset))
      case PeriodData(_) =>
        period =>
          Json.jString(period.toString)
      case YearData(_) =>
        year =>
          Json.jNumber(year.getValue())
      case YearMonthData(_) =>
        yearMonth =>
          Json.jString(yearMonth.toString)
      case ZoneIdData(_) =>
        zoneId =>
          Json.jString(zoneId.toString)
      case ZonedDateTimeData(_) =>
        dateTime =>
          Json.jString(zonedDateTimeFormatter.format(dateTime))
      case ZoneOffsetData(_) =>
        zoneOffset =>
          Json.jString(zoneOffset.toString)
    }
}
