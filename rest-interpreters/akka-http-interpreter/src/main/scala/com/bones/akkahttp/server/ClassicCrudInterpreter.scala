...
object ClassicCrudInterpreter {
  def empty[ALG[_], ID: Manifest, A: Manifest, E](
    ...
    implicit executionContext: ExecutionContext): ClassicCrudInterpreter[ALG, ID, A, E] =
    ClassicCrudInterpreter(config, path, schema, pathStringToId, errorSchema)
...
case class ClassicCrudInterpreter[ALG[_], ID: Manifest, A: Manifest, E](
  config: InterpreterConfig[ALG, ID],
  urlPath: String,
  schema: KvpCollection[String, ALG, A],
  pathStringToId: String => Either[String, ID],
  ...
  val validationErrorEncoder =
    ClassicCrudInterpreter.isoSprayEncoderInterpreter.generateEncoder(
  ...
  def withCreate(f: A => Future[Either[E, ID]]): ClassicCrudInterpreter[ALG, ID, A, E] =
    this.copy(createF = Some(f))

  def withUpdate(f: (ID, A) => Future[Either[E, ID]]): ClassicCrudInterpreter[ALG, ID, A, E] =
    this.copy(updateF = Some(f))

  def withDelete(f: ID => Future[Either[E, (ID, A)]]): ClassicCrudInterpreter[ALG, ID, A, E] =
    this.copy(deleteF = Some(f))

  def withRead(f: ID => Future[Either[E, (ID, A)]]): ClassicCrudInterpreter[ALG, ID, A, E] =
...
//  def routes: Route = {}
  ...
  def routeForGet(f: ID => Future[Either[E, (ID, A)]]): Route = {
    pathPrefix(urlPath / Remaining) { idString =>
      get {
        val futureResult = pathStringToId(idString) match {
          case Left(err) =>
            Future(
                  ...
                  fResult match {
                    case Left(systemError) =>
                      complete(
                        ...
                        errorMarshallerFunction(systemError))
                    case Right(value) =>
