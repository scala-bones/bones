...
import com.gaia.soy
import com.gaia.soy.{Key, JsonProducer, RootKey, StringKey, WrongTypeError}
...
case class LiftJson(jValue: JValue) extends JsonProducer {
  ...
  override def produceBigDecimal(key: soy.Key): Either[WrongTypeError[BigDecimal], Option[BigDecimal]] = key match {
    case RootKey => lookForBigDecimal(key, jValue)
    case StringKey(field) => lookForBigDecimal(key, jValue \ field)
  }


  override def produceString(key: soy.Key): Either[WrongTypeError[String], Option[String]] = key match {
    case RootKey => lookForString(key, jValue)
    case StringKey(field) => lookForString(key, jValue \ field)
  }

  override def produceInt(key: soy.Key): Either[WrongTypeError[Int], Option[Int]] = key match {
    case RootKey => lookForInt(key, jValue)
    case StringKey(field) => lookForInt(key, jValue \ field)
  }

  override def produceObject(key: soy.Key): Either[WrongTypeError[JsonProducer], Option[LiftJson]] = key match {
    case RootKey => lookForObject(key, jValue)
    case StringKey(field) => lookForObject(key, jValue \ field)
  }


  override def produceBool(key: soy.Key): Either[WrongTypeError[Boolean], Option[Boolean]] = key match {
